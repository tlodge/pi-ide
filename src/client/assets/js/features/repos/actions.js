...
const _generateManifest = (app, reponame, packages, nodes, username) => {
  ...
  console.log("useranme is", uname);
  console.log("in generate manifest with app", app);
  console.log("and repo name", reponame);

  const appname = app.name.startsWith(uname) ? app.name : `${uname}-${app.name}`;
  reponame = reponame && reponame.trim() != "" ? reponame : appname;

  console.log("setting app name to", appname);
  console.log("and repo name to", reponame);
